...
<style>
    ...
    .icon-box-blue { background: #eef2ff; color: #4c6ef5; }
    .icon-box-green { background: #f0fdf4; color: var(--primary-mint); }
    .icon-box-purple { background: #f5f3ff; color: #7c3aed; }

    .btn-verify {
        background: linear-gradient(135deg, #e11d48, #be123c); /* Warna merah untuk Keluar */
        color: white;
        border: none;
        width: 100%;
        padding: 20px;
                    ...
                    <div class="col-lg-7">
                        <div class="mb-4 d-flex align-items-center gap-3">
                            <div style="background: #fff1f2; padding: 10px; border-radius: 12px;">
                                <i class="ti ti-logout fs-2" style="color: #e11d48;"></i>
                            </div>
                            <h2 style="font-family: 'JetBrains Mono'; font-weight: 700; color: var(--dark-surface); text-transform: uppercase; margin: 0; font-size: 1.4rem;">Presensi Keluar</h2>
                        </div>
                        ...
                        <div class="camera-viewport">
                            ...
                            <div class="tech-label">
                                <span style="color: #ff4d4d;">●</span> EXIT_PROTOCOL_READY
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-5">
                        <div class="side-panel">
                            ...
                            <div class="mt-4">
                                ...
                                <button id="ambil-foto-keluar" class="btn-verify">
                                    <i class="ti ti-power fs-3"></i>
                                    <span>Verifikasi Keluar</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
Webcam.on('live', function() {
    ...
    if (videoWrap) { videoWrap.style.width = '100%'; videoWrap.style.height = '100%'; }
    if (videoEl) { 
...
document.getElementById('ambil-foto-keluar').addEventListener('click', function(){
    ...
    const jam_keluar = document.getElementById('jam_keluar').value;

    // KUNCI PERBAIKAN: Ambil ID Presensi dari variabel PHP
    const id_presensi = "<?= $id_presensi ?>"; 
    ...
    var xhttp = new XMLHttpRequest();
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4) {
                if (this.status == 200) {
                    // BERHASIL
                    btn.innerHTML = '<i class="ti ti-circle-check fs-3"></i> SELESAI!';
                    ...
                    // GAGAL (Contoh: 404 atau 500)
                    alert("Gagal mengirim data! Status: " + this.status);
                    
                    // Kembalikan tombol ke semula agar user bisa coba lagi
                    btn.disabled = false;
                    btn.style.background = 'linear-gradient(135deg, #e11d48, #be123c)';
                    btn.innerHTML = '<i class="ti ti-power fs-3"></i> <span>Verifikasi Keluar</span>';
                }
            }
        };

        // KUNCI PERBAIKAN: URL harus menyertakan ID Presensi
